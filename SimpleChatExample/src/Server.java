 ...
 * tradutor: enter(=leolucass) (alguns comentários foram traduzidos, para facilitar estudo)
 *O server pode ser executado como aplicação ou como GUI
 ...
 */
public class Server {
	// Uma ID única para cada conexão
	...
	// O número da porta para listen for connection (comunicar/ser comunicado sobre a conexão)
	private int port;
	...
	public void start() {
		keepGoing = true;
		/* cria o socket(=encaixe) server e espera por requisição de conexão (connection requests)*/
		try 
		{
			// O socket usado pelo server
			ServerSocket serverSocket = new ServerSocket(port);

			// loop infinito para esperar por conexões
			while(keepGoing) 
			{
				//formato da mensagem dizendo que estamos esperando.
				display("Server waiting for Clients on port " + port + ".");
				
				Socket socket = serverSocket.accept();  	// conexão aceita
			...
			try {
				...
				for(int i = 0; i < al.size(); ++i) {
					...
					tc.socket.close();
						...
						// not much I can do : Não pode-se fazer muita coisa
					}
				}
			}
			catch(Exception e) {
	...
	private synchronized void broadcast(String message) {
		...
		if(sg == null)
			...
			sg.appendRoom(messageLf);     //Anexar na sala/espaço da janela  append in the room window
